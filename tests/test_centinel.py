...
@patch('nomad.api.quotas.Quotas._get')
def test_list_policies(mock_get, nomad_setup):
    ...
    policies = nomad_setup.quotas.get_policies()
    assert isinstance(policies, list)
    assert "foo" in nomad_setup.quotas.get_policies()[0]["Name"]

@patch('nomad.api.quotas.Quotas._post_no_json')
def test_create_policy(mock_post_no_json, nomad_setup):
    ...
    assert 200 == nomad_setup.quotas.create_policy(id="my-policy", policy=json_policy)
...
def test_update_policy(mock_post_no_json, nomad_setup):
    ...
    json_policy = json.loads(policy_example)
    assert 200 == nomad_setup.quotas.update_policy(id="my-policy", policy=json_policy)
...
def test_get_policy(mock_get, nomad_setup):
    ...
    policy = nomad_setup.quotas.get_policy("foo")
    assert "advisory" in policy["EnforcementLevel"]

@patch('nomad.api.quotas.Quotas._delete')
def test_delete_policy(mock_delete, nomad_setup):
    mock_delete.return_value = requests.codes.ok
    nomad_setup.quotas.delete_policy(id="my-policy")
    assert 200 == nomad_setup.quotas.delete_policy(id="my-policy")
